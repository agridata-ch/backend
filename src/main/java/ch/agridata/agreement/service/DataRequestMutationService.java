...
public class DataRequestMutationService {
  ...
  private DataRequestDto updateEntityWithDto(DataRequestUpdateDto dataRequestDto,
    ...
    validateDataProducts(dataRequestDto);
  ...
  private void validateDataProducts(@NotNull DataRequestUpdateDto dataRequestDto) {
    if (dataRequestDto.products() == null) {
      return;
    }
    for (UUID product : dataRequestDto.products()) {
      try {
        dataProductApi.getProductById(product);
      } catch (NotFoundException e) {
        throw new IllegalStateException(
            "Cannot process request: data product " + product + " not found", e
        );
      }
    }
  }
