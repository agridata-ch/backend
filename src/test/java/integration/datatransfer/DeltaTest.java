...
class DeltaTest {
  ...
  void givenGrantedConsents_whenDeltaRequested_thenIdsReturned() {
    ...
    AuthTestUtils.requestAs(PRODUCER_032).contentType(ContentType.JSON)
        ...
        .put(ConsentRequestController.PATH + "/" + TestDataIdentifiers.ConsentRequest.BIO_SUISSE_01_CHE_435 + "/status")
        .then().statusCode(204);

    AuthTestUtils.requestAs(PRODUCER_032).contentType(ContentType.JSON)
        .body(String.format("\"%s\"", GRANTED)).when()
        .put(ConsentRequestController.PATH + "/" + TestDataIdentifiers.ConsentRequest.BIO_SUISSE_01_CHE_860 + "/status")
        .then().statusCode(204);

    assertThat(getDeltaIds(testStartTime)).containsExactlyInAnyOrderElementsOf(List.of(CHE435, CHE860));
  }

  private List<String> getDeltaIds(LocalDateTime since) {
    return AuthTestUtils.requestAs(CONSUMER_BIO_SUISSE)
        .pathParam("productId", TestDataIdentifiers.DataProduct.UUID_C661EA48.uuid())
